...
class ApiService {
  ...
  constructor() {
    ...
    console.log(`[ApiService] Mode: ${this.useMockApi ? 'Mock' : 'Real'}, Base URL: ${this.baseUrl}`);
  }
  ...
  private isSelHosted(): boolean {
    ...
    return document.documentElement.getAttribute('data-deployment') === 'self-hosted' ||
  ...
  async call<T>(endpoint: string, options: RequestInit = {}): Promise<ApiResponse<T>> {
  ...
  private async realApiCall<T>(endpoint: string, options: RequestInit): Promise<ApiResponse<T>> {
    try {
      ...
      console.error(`[ApiService] Error calling ${endpoint}:`, error);
      return { 
        data: undefined, 
  ...
  enableMockMode(): void {
    ...
    console.log('[ApiService] Switched to Mock API mode');
  }
  ...
  enableRealMode(): void {
    ...
    console.log('[ApiService] Switched to Real API mode');
  }
