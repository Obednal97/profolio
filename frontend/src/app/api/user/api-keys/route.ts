...
function getUserFromToken(
  request: NextRequest
): { userId: string; email: string; isDemo: boolean } | null {
  try {
    ...
    for (const [provider, encryptedData] of Object.entries(encryptedKeys)) {
      try {
        ...
        if (process.env.NODE_ENV === "development") {
          ...
          console.error(
            `Failed to decrypt ${provider} API key: Decryption failed`
          );
        }
      ...
      if (typeof apiKey === "string" && apiKey.trim()) {
        try {
          ...
          if (process.env.NODE_ENV === "development") {
            ...
            console.error(
              `Failed to encrypt ${provider} API key: Encryption failed`
            );
          }
          return NextResponse.json(
