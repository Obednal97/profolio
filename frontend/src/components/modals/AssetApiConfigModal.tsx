  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <motion.div
        ...
        className="bg-white dark:bg-gray-800 rounded-2xl p-6 w-full max-w-3xl max-h-[90vh] overflow-y-auto border border-gray-200 dark:border-gray-700 shadow-2xl"
      >
        {/* Header */}
        ...
        <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4 mb-6">
          ...
          <p className="text-sm text-blue-700 dark:text-blue-400">
            Connect your financial data providers to automatically sync portfolio data, get real-time prices, and access advanced market information.
          </p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-6">
          {apiProviders.map((provider) => (
            <div key={provider.key} className="bg-gray-50 dark:bg-gray-700/50 rounded-xl p-4 space-y-3">
              <div className="flex items-center justify-between">
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 flex items-center">
                  <i className={`${provider.icon} mr-2 text-${provider.color}-500`}></i>
                  {provider.label}
                </label>
                <a
