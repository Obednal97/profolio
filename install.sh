...
main() {
    ...
    if [[ "${DRY_RUN_MODE}" == "true" ]]; then
        ...
        # Safely get professional features count without causing pipefail exit
        local prof_features=$(info 2>/dev/null | grep -o '[0-9]\+/[0-9]\+' | tail -1 2>/dev/null || echo "Unknown")
        echo -e "${BLUE}Professional Features:${NC} $prof_features"
