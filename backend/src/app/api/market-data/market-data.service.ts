...
export class MarketDataService {
  ...
  async getCurrentPrice(userId: string, symbol: string, assetType: 'STOCK' | 'CRYPTO'): Promise<PriceData | null> {
    try {
      if (assetType === 'CRYPTO') {
  ...
  async getHistoricalData(userId: string, symbol: string, assetType: 'STOCK' | 'CRYPTO', days: number = 30): Promise<HistoricalData | null> {
    try {
      if (assetType === 'CRYPTO') {
  ...
  private async getCryptoPrice(userId: string, symbol: string): Promise<PriceData | null> {
  ...
  private async getStockPrice(userId: string, symbol: string): Promise<PriceData | null> {
  ...
  private async fetchCoinGeckoPrice(symbol: string, apiKey?: string): Promise<PriceData | null> {
    try {
      const headers: any = {};
      if (apiKey) {
  ...
  private async fetchAlphaVantagePrice(symbol: string, apiKey: string): Promise<PriceData | null> {
    try {
      const response = await fetch(
  ...
  private async fetchTwelveDataPrice(symbol: string, apiKey: string): Promise<PriceData | null> {
    try {
      const response = await fetch(
  ...
  private async getCryptoHistoricalData(userId: string, symbol: string, days: number): Promise<HistoricalData | null> {
    const coinGeckoKey = await this.apiKeysService.findActiveByProvider(userId, 'COINGECKO');
  ...
  private async getStockHistoricalData(userId: string, symbol: string, days: number): Promise<HistoricalData | null> {
    const alphaVantageKey = await this.apiKeysService.findActiveByProvider(userId, 'ALPHA_VANTAGE');
