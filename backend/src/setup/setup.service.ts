...
export class SetupService {
  ...
  async initializeApplication(config: SetupConfigDto) {
    try {
      ...
      const jwtSecret = config.security.jwtSecret || this.generateSecureSecret(32);
      const encryptionKey = config.security.encryptionKey || this.generateSecureSecret(32);
      ...
      await this.updateEnvironmentConfig({
        ...
        REQUIRE_EMAIL_VERIFICATION: config.features.requireEmailVerification.toString(),
      });
  ...
  private async storeInitialApiKeys(apiKeys: Record<string, string>, userEmail: string) {
    try {
      ...
      // Store API keys (you'd use the ApiKeysService here)
      for (const [provider, key] of Object.entries(apiKeys)) {
        if (key && key.trim()) {
          this.logger.log(`Storing API key for provider: ${provider}`);
          // TODO: Use ApiKeysService.create() here when available
        }
      }

    } catch (error) {
      this.logger.error('Failed to store initial API keys:', error);
      // Don't throw here - API keys are optional
    }
  }

  private async markSetupComplete() {
    await fs.promises.writeFile(this.setupFlagPath, new Date().toISOString(), 'utf8');
  }

  // Method to check if this is a self-hosted deployment
  isSelHosted(): boolean {
    return process.env.DEPLOYMENT_TYPE === 'self-hosted' || 
